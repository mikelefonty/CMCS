...
if __name__ == "__main__":
    ...
    parser.add_argument("--blocks", type=int, default= 1,
                        help="Number of blocks for the Block Simulator (default = 1)")

    parser.add_argument("--smart",action="store_true",
                        help="Use the smart simulators")
    ...
    try:
        ...
        if args.smart:
            result_name_std = result_name+'_std_'+str(args.blocks)+'_smart'
            result_name_block = result_name+'_block_'+str(args.blocks)+'_smart'
        else:
            result_name_std = result_name+'_std_'+str(args.blocks)
            result_name_block = result_name+'_block_'+str(args.blocks)
        
        standard_sim = Simulator(env_dir+"/"+env_path,use_nn=args.smart,
        ...
        block_sim.simulate(args.n_iters,verbose=args.verbose)

        if args.replay:
            replicate_simulation_from_file(results_path+"/"+result_name_block)

        df_std = pd.read_json(results_path+"/"+result_name_std+"_measures.json")
        df_smart = pd.read_json(results_path+"/"+result_name_block+"_measures.json")
        
        plt.figure()

        df_meta = pd.read_json(results_path+"/"+result_name_std+"_meta.json")
        
        title_template = 'Confronto Simulatore Standard vs Block\n Ambiente : {} {}x{}\nNeigh_Size = {} Selezione agenti: {}\n Simulazione {}'.format(
        ...
        plt.plot(df_smart['Iterazione'],df_smart['Misura'],label='Simulatore a Blocchi')
        ...
        if args.smart:
            plot_title = results_path+"/"+result_name+"_smart_"+str(args.blocks)+"_confronto_plot.png"
        else:
            plot_title = results_path+"/"+result_name+"_"+str(args.blocks)+"_confronto_plot.png"
        plt.savefig(plot_title,bbox='tight')
