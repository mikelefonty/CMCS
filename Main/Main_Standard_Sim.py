...
if __name__ == "__main__":
    ...
    try:
        ...
        sim = Simulator(env_dir+"/"+env_path,use_nn=args.smart,
        show_anim=args.show_anim,seed=seed,use_random_selection=args.rand_strat,neigh_size=args.k,
        use_stochastic_sim=args.stoc, log_path=results_path, log_file=result_name)
        print("EHIII")
        ...
        df = pd.read_json(results_path+"/"+result_name+"_measures.json")
     
        plt.figure()

        df_meta = pd.read_json(results_path+"/"+result_name+"_meta.json")
        
        title_template = 'Simulatore {}\n Ambiente : {} {}x{}\nNeigh_Size = {} Selezione agenti: {}\n Simulazione {}'.format(
            df_meta.iloc[0]['Tipo_Simulatore'],df_meta.iloc[0]['Nome_Ambiente'],df_meta.iloc[0]['Numero_Righe'],
        ...
        plt.plot(df['Iterazione'],df['Misura'],label='Simulatore {}'.format(df_meta.iloc[0]['Tipo_Simulatore']))
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(results_path+"/"+result_name+"_plot.png",bbox='tight')
